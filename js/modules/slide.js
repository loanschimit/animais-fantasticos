...
export class Slide {
  ...
  // (4) determina que finalPosition será igual ao this.updatePosition(event.clientX)
  // e executa moveSlide(finalPosition)
  onMove(event) {
  ...
  onEnd(event) {
    ...
    // determina que a posição final vai ser a do próximo inicio
  ...
  slidePosition(slide) {
    ...
    // tamanho do wrapper menos o tamanho do slide(argumento), = margin dividido em 2
    return -(slide.offsetLeft - margin);
    // posição lateral menos a margin, assim a posição sera centralizada
    // (o valor precisa ser negativo)
  }

  // (5) Dita o posicionamento com as configurações
  slidesConfig() {
    // ↓ loop da array com position sendo constante de
    // slidePosition(element) ← e element sendo o argumento passado
  ...
  changeSlide(index) {
    ...
    // constante que determina a posição em relação ao index da array
    this.moveSlide(activeSlide.position);
    // executa moveSlide com a constante activeSlide.position como argumento
    this.slideIndexNav(index);
    // determina qual será o próximo o atual e o anterior com configurações especificas
    this.dist.finalPosition = activeSlide.position;
    // determina que a posição final seja igual a activeSlide.position
...
export default class SlideNav extends Slide {
  ...
  // adiciona o evento de clique aos botões, executando activePrevSlide e activeNextSlide
  // que são responsáveis por navegar de proximo para anterior
  // addEventArrow() {
  ...
  createControl() {
    ...
    // ↓ faz o loop em slideArray adicionando ao control.innerHTML um a dentro de uma li,
    // contendo href de #slide com index de cada item de slideArray
  ...
  // adiciona um evento de clique ao item, previne o padrão,
  // e executa changeSlide com o index e activeControlItem
  eventControl(item, index) {
    ...
    // determina que quando ocorrer o changeEvent executara o activeControlItem,
    // para quando ocorrer as mudanças no slide a navegação acompanhar
    this.wrapper.addEventListener("changeEvent", this.activeControlItem);
  }

  // remove a classe de todos os itens através do forEach
  // e adiciona a classe ao item que será estilizado
  activeControlItem() {
  ...
  // executa createControl() ou o custom que for criado, desestrutura ele como array(controlArray),
  // faz um loop em controlArray executando eventControl()
